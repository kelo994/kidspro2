...
export class SkillService {
    ...
    getSkillsPerUnits(unitId) {
        return this.http.
        get(`${this.url}/admin/units/${unitId}/skills`)
            .pipe(timeout(5000),
                retry(1),
                catchError((error, c) => {
                    this.errorTime();
    ...
    save(data: any) {
        return this.http.post(`${this.url}/admin/skills`, data)
            .pipe(
                catchError((error, c) => {
                    this.errorTime();
                    return throwError(error);
    ...
    delete(id: any) {
        return this.http.delete(`${this.url}/admin/skills/${id}`)
            .pipe(
                catchError((error, c) => {
                    this.errorTime();
                    return throwError(error)
