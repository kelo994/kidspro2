...
export class BlockService {
    ...
    getBlocksPerUnit(unitId) {
        return this.http.
        get(`${this.url}/admin/units/${unitId}/blocks`)
            .pipe(timeout(5000),
                retry(1),
                catchError((error, c) => {
                    this.errorTime();
    ...
    save(data: any) {
        return this.http.post(`${this.url}/admin/blocks`, data)
            .pipe(
                catchError((error, c) => {
                    this.errorTime();
                    return throwError(error);
    ...
    delete(id: any) {
        return this.http.delete(`${this.url}/admin/blocks/${id}`)
            .pipe(
                catchError((error, c) => {
                    this.errorTime();
                    return throwError(error)
